...
def paired_paths_from_txt(txts, keys, filename_tmpl, name):
    ...
    if name=='animeFaces-4':
        ...
        # 单光传输条件
        label_dict_train = {"C_dark_1_d100_occluder": 0}

    elif name=='single':
        #单独训练
        label_dict_train = {"C_dark_1_d70_wb": 0, "C_dark_1_d100_wall": 0, "C_dark_2_d70_wall": 0, "C_dark_2_d70_wb": 0, 
                            "C_dark_2_d100_wall": 0, "C_dark_2_d100_wb": 0, "C_day_1_d70_wall": 0, "C_day_1_d70_wb": 0,
                            "C_day_2_d70_wall": 0, "C_dark_1_d70_wall": 0, "C_dark_1_d100_wb": 0}
    # 单光传输条件
    #label_dict_train = {"C_dark_1_d100_wall": 0}
    assert len(txts) == 2, (
    ...
    for idx in range(len(gt_paths)):
        if name=='STL10':
            gt_path = gt_paths[idx] #  'AONLOS/Dataset/chaomo/B_flip/train/6333_1.png'
            #basename, ext = osp.splitext(osp.basename(gt_path)) # basename='6333_1', ext='.png'
            input_path = input_paths[idx] # 'AONLOS/supermodelFaces/C_day_1_d100_wall/train/6333_1.png'
            #basename_input, ext_input = osp.splitext(osp.basename(input_path)) # basename='6333_1', ext='.png'
            #input_name = f'{filename_tmpl.format(basename)}{ext_input}' # '6333_1.png
            # datasets/AONLOS/STL10/stl10_genera/chaomo_withstl10/testchaomo_17.png
            input_path = osp.join(input_path)
            #assert input_name in input_paths, (f'{input_name} is not in '
            #                                   f'{input_key}_paths.')
            gt_path = osp.join(gt_path)
            label = label_dict_train["C_dark_1_d100_occluder"]
            paths.append(
            ...
            #   label = label_dict_train[input_path.split('/')[4][:-10]] #在ustc采集的mnist数据集 
            paths.append(
                dict([(f'{input_key}_path', input_path),
                    (f'{gt_key}_path', gt_path),
                    (f'{label_key}', label)]))
