...
class ImageRestorationModel(BaseModel): # 之前因为ImageRestorationModel继承的不是torch.nn.Module类所以不能to(device),也不能callable
    ...
    def compute_generator_loss(self):
        ...
        for i in range(num_D):  # for each discriminator
            ...
            for j in range(num_intermediate_outputs):  # for each layer output
                unweighted_loss = self.criterionFeat(
                        pred_fake[i][j], pred_real[i][j].detach()) # 重模糊图像和真实图像通过鉴别器，lq和真实图像通过鉴别器之间的l1 loss
        ...
        l_pix = 0.
        #for pred in preds:
            #l_vgg += 10 *self.criterionVGG(pred, self.gt)
    ...
    def discriminate(self, input_semantics, fake_image, real_image):
        fake_concat = torch.cat([input_semantics, fake_image], dim=1) # 真实图像和重模糊图像
        real_concat = torch.cat([input_semantics, real_image], dim=1) # 真实图像和投影图像
    ...
    def compute_discriminator_loss(self):
        ...
        D_losses['D_Fake'] = self.criterionGAN(pred_fake, False,
                                               for_discriminator=True) # 重模糊图像和真实图像，鉴别器需判断重模糊图像为fake
        D_losses['D_real'] = self.criterionGAN(pred_real, True,
                                               for_discriminator=True) # 投影图像和真实图像，鉴别器需判断投影图像为real
        return D_losses
    ...
    def validation_test(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            s1_img = tensor2img(visuals[1].data)
            sr_img = tensor2img(visuals[2].data)
            re_img = tensor2img(visuals[3].data) # gt为None时，visuals[2]也为None
